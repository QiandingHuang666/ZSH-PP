...
int main(int argc, char** argv) {
    ...
    if (argc == 6)
    {
        ...
        if (!cap.isOpened()) {
            std::cout << "\033[1;31m" << "[ Deploy Laplacian Filter on GPU ]" << "\033[0m\n"
                      << " [Camera/Video] ERRO.\n";
            return 0;
        }
    ...
    else 
    {
        std::cout << "\033[1;31m" << "[ Deploy Laplacian Filter on GPU ]" << "\033[0m\n"
                  ...
                  << "\033[1;31m[ Deploy Laplacian Filter on GPU ]\033[0m\n";
        return 0;
    }
    ...
    CPULaplacianFilter filter(numThread);
    ...
    printf("\n\033[1;32m[ Deploy Laplacian Filter on GPU ]\033[0m Cam FPS: %10.4f\n",
                cam_fps);
    printf("\033[1;32m[ Deploy Laplacian Filter on GPU ]\033[0m Min FPS: %10.4f\n",
                1/times.max_time);
    printf("\033[1;32m[ Deploy Laplacian Filter on GPU ]\033[0m Max FPS: %10.4f\n",
                1/times.min_time);
    printf("\033[1;32m[ Deploy Laplacian Filter on GPU ]\033[0m Avg FPS: %10.4f\n\n",
                times.num_frame/times.time);

    if (saveOrigin)
        printf("\033[1;32m[ Deploy Laplacian Filter on GPU ]\033[0m"
