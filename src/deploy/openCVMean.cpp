...
int main(int argc, char** argv) {
    ...
    std::string outputVideoPath("output/Video/MeanVideo.mp4");
    ...
    if (argc == 6)
    {
        ...
        if (!cap.isOpened()) {
            std::cout << "\033[1;31m" << "[ Deploy Mean Filter on OpenCV ]" << "\033[0m\n"
                      << " [Camera/Video] ERRO.\n";
        }
    ...
    else 
    {
        std::cout << "\033[1;31m" << "[ Deploy Mean Filter on OpenCV ]" << "\033[0m\n"
                  ...
                  << "\033[1;31m[ Deploy Mean Filter on OpenCV ]\033[0m\n";
        return 0;
    }

    if (openOrigin)
        cv::namedWindow("Origin", 1);
    cv::namedWindow("Deploy Mean Filtering", 1);
    ...
    OpenCVMeanFilter filter(kernelSize);
    ...
    if (!saveOutput)
        outputVideoPath = "output/Video/MeanVideo.cache";
    ...
    while (true) {
        ...
        cv::putText(deploy_frame, buffer, cv::Point(15, 22), 
                    ...
                    cv::Scalar(0, 255, 0), 2);
        cv::imshow("Deploy Mean Filtering", deploy_frame);
    ...
    printf("\n\033[1;32m[ Deploy Mean Filter on OpenCV ]\033[0m Cam FPS: %10.4f\n",
                cam_fps);
    printf("\033[1;32m[ Deploy Mean Filter on OpenCV ]\033[0m Min FPS: %10.4f\n",
                1/times.max_time);
    printf("\033[1;32m[ Deploy Mean Filter on OpenCV ]\033[0m Max FPS: %10.4f\n",
                1/times.min_time);
    printf("\033[1;32m[ Deploy Mean Filter on OpenCV ]\033[0m Avg FPS: %10.4f\n\n",
                times.num_frame/times.time);

    if (saveOrigin)
        printf("\033[1;32m[ Deploy Mean Filter on OpenCV ]\033[0m"
               " Origin Video has been saved in " 
               "\033[1;36moutput/Video/originVideo.mp4\033[0m.\n");
    if (saveOutput)
        printf("\033[1;32m[ Deploy Mean Filter on OpenCV ]\033[0m"
               " Output Video has been saved in " 
               "\033[1;36moutput/Video/MeanVideo.mp4\033[0m.\n");
