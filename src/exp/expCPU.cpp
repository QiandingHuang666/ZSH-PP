...
int main(int argc, char** argv) {
    if (argc < 2 || argc > 7)
    {
        std::cout << "\033[1;31m" << "[ Experiment on CPU ]" << "\033[0m\n"
                  ...
                  << "\t[Kernel Size]\n"
                  << "\t[Sigma]\n" 
                  << "\033[1;31m[ Experiment on CPU ]\033[0m\n";
        return 0;
    }
    ...
    if (filter == "gauss") {
        sigma = std::stod(argv[6]);
        kernelSize = std::stoi(argv[5]);
        filterType = 0;
    }
    else if (filter == "mean") {
        kernelSize = std::stoi(argv[5]);
    ...
    std::cout <<  "\033[1;32m" << "[ Experiment on CPU ]" << "\033[0m"
    ...
    cv::Mat out_img;
     for (auto it = intputImgs.begin(); it != intputImgs.end(); ++it) {
        ...
        times.time += seconds;
        times.num_frame++;
        cv::imwrite(result_path + "[CPU]_[" + filter +
                        "]_Thread[" + std::to_string(numThread) +
                        "]_kernelSize[" + 
    ...
    printf("\n\033[1;32m[ Experiment on CPU ]\033[0m Num of Thread:\033[1;34m%10d\033[0m\n", 
           numThread);
    printf("\033[1;32m[ Experiment on CPU ]\033[0m Kernel   Size:\033[1;34m%10d\033[0m\n", 
           kernelSize);
    printf("\033[1;32m[ Experiment on CPU ]\033[0m Max       FPS:\033[1;34m%10.4f\033[0m\n", 
           1/times.min_time);
    printf("\033[1;32m[ Experiment on CPU ]\033[0m Min       FPS:\033[1;34m%10.4f\033[0m\n", 
           1/times.max_time);
    printf("\033[1;32m[ Experiment on CPU ]\033[0m Avg       FPS:\033[1;34m%10.4f\033[0m\n\n", 
